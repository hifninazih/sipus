<?php
	...
	if(!isset($_POST['cari'])) { // Jika tidak melakukan pencarian kategori
		...
		/* Query Count Belum Kembali */
		$sql_count_belum = "SELECT transaksi.id_transaksi
					  FROM transaksi
					  WHERE transaksi.tanggal_kembali IS NULL;";
		$query_count_belum = mysqli_query($db_conn, $sql_count_belum);
		$hasil_belum = 'Belum Kembali : ';		
		$row_belum = $query_count_belum->num_rows;
			...
			<div class="page-content">
				<div class="table-upper">
					<div class="table-upper-left">
						<a href="app/semua-cetak-daftar.php" title="Cetak Semua Daftar Transaksi" target="_blank" class="btn-primary btn-medium">Cetak</a>
						<a href="index.php?p=semua-transaksi" class="btn-info btn-medium ml-2">Refresh</a>
					</div>
			...
			?>
				...
				?>
					<tr>
						...
						<td class="text-center"><?php if($data['tanggal_kembali'] != null){ echo $data['tanggal_kembali'];} else { echo '-';} ?></td>
						<td class="text-center"><?php if($data['tanggal_kembali'] == null){ echo '<span class="badge badge-secondary">Belum Kembali</span>';} else { echo '<span class="badge badge-success">Sudah Kembali</span>';} ?></td>
				...
				<div class="table-lower">
					<div class="table-lower-left mg-top-5">
						<?= $hasil_belum . $row_belum .'<br>'. $hasil . $row; ?>
					</div>
